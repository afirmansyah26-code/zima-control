...
test("authority package has no private issuer key, trust writer, provisioning, Docker, or runtime topology capability", async () => {
  ...
  assert.deepEqual(Object.keys(manifest.dependencies).sort(), ["@zima-control-center/runtime-trust-contracts"]);
  ...
  assert.doesNotMatch(snapshot, /\.create\(|\.update\(|\.delete\(|\$executeRaw/);
  const rawQueries = snapshot.split(/\r?\n/)
    .filter((line) => line.includes("await transaction.$queryRawUnsafe"))
    .map((line) => /\("([^"]+)"\);$/.exec(line.trim())?.[1] ?? "DYNAMIC_OR_MALFORMED");
